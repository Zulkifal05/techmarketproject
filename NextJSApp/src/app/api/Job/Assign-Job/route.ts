...
export async function PATCH(req: Request) {
    try {
        ...
        const { jobId, proposalId } = body

        if (!jobId || !proposalId) {
            return NextResponse.json({ message: "Job ID and proposal ID are required", success: false }, { status: 400 })
        }

        const validationResult = AssignJobSchema.safeParse(body)
